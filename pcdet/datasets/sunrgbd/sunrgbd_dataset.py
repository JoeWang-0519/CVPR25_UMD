...
class SunrgbdDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None, filter_empty_gt=True,):
        ...
        # print(self.cat2id)
        # sample_id_list: index list
        self.sample_id_list = []
        # infos: label list
        self.sunrgbd_infos = []
        self.filter_empty_gt = filter_empty_gt
        ## load the sunrgbd 'info list' and 'sample index list' for __getitem__()
        self.include_sunrgbd_data(self.mode)
        ## init the data augmentor for training and testing in prepare()
    ...
    def include_sunrgbd_data(self, mode):
        if self.logger is not None:
            self.logger.info('Loading the whole SUNRGBD dataset')
        sunrgbd_infos = []
        sample_id_list = []
    ...
    def prepare_data(self, data_dict):
        ...
        if self.training:
            ...
            ## according to cfg (CAGroup3D), filter out those unintersted bounding boxes
            gt_boxes_mask = np.array([n in self.class_names for n in data_dict['gt_names']], dtype=np.bool_)

            ## filter out unintersted classes, shift the heading anchor range, data augmentation
        ...
        ## just make use of the [x,y,z,r,g,b]
        if data_dict.get('points', None) is not None:
            data_dict = self.point_feature_encoder.forward(data_dict)

        # no processor (mask out of range points)
    ...
    def __getitem__(self, index):
        ...
        if "points" in get_item_list:
            # sample_idx can retrieve the corresponding points
        ...
        ## added
        if "fn_number" in info:
            fn_number = info['fn_number']
            input_dict['fn_number'] = fn_number
        
        if "pred_boxes" in info:
            pred_boxes = info['pred_boxes']
            input_dict['pred_boxes'] = pred_boxes

        # if "pred_labels" in info:
        #     pred_labels = info['pred_labels']
        #     input_dict['pred_labels'] = pred_labels

        if "global_feat" in info:
            global_feat = info['global_feat']
            input_dict['global_feat'] = global_feat

        if "detected_feat" in info:
            detected_feat = info['detected_feat']
            input_dict['detected_feat'] = detected_feat

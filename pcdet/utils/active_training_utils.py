...
def select_active_labels(model,
    ...
    strategy = query_strategies.build_strategy(method=method, model=model, \
                                               ...
                                               cfg=cfg)
    if os.path.isfile(os.path.join(active_label_dir, 'selected_frames_epoch_{}_rank_0.pkl'.format(cur_epoch))):
        # if found, then resume

        # TODO: needed to be revised
        print('found {} epoch saved selections...start resuming...'.format(cur_epoch))
        with open(os.path.join(active_label_dir, 'selected_frames_epoch_{}_rank_0.pkl'.format(cur_epoch)), 'rb') as f:
            selected_frames = pkl.load(f)

    else:

        selected_frames = strategy.query(leave_pbar, cur_epoch, record_epoch, logger)
        # 要修改post processsing of cagroup3d
    ...
    if cfg.DATA_CONFIG.DATASET == 'ScannetDataset': # scannet
        selected_id_list, selected_infos = list(strategy.labelled_set.sample_id_list), \
            list(strategy.labelled_set.scannet_infos)
        unselected_id_list, unselected_infos = list(strategy.unlabelled_set.sample_id_list), \
            list(strategy.unlabelled_set.scannet_infos)
    else:  # sunrgbd
        selected_id_list, selected_infos = list(strategy.labelled_set.sample_id_list), \
            list(strategy.labelled_set.sunrgbd_infos)
        unselected_id_list, unselected_infos = list(strategy.unlabelled_set.sample_id_list), \
            list(strategy.unlabelled_set.sunrgbd_infos)

    ## strategy.pairs: unlabelled pairs
    ## equivalently, it contains previous 'unselected_id_list + unselected infos'
    ## update the selected & unselected list based on active samples
    ...
    ## active_training: updated selected & unselected list
    ...
    ## obtain the dataloader & dataset & sampler based on updated selected & unselected samples

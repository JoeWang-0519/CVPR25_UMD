...
class Strategy:
    def __init__(self, model, labelled_loader, unlabelled_loader, rank, active_label_dir, cfg):
        ...
        if cfg.DATA_CONFIG.DATASET == 'ScannetDataset': # scannet case
            self.pairs = list(zip(self.unlabelled_set.sample_id_list, self.unlabelled_set.scannet_infos))
        else: # sunrgbd case
            self.pairs = list(zip(self.unlabelled_set.sample_id_list, self.unlabelled_set.sunrgbd_infos))
    ...
    def update_dashboard(self, cur_epoch=None, accumulated_iter=None):
        ...
        for cls_idx in classes:
            ...
            if num_cls_bbox:
                for met in self.point_measures:
                    stats_point = sum([i[cls_idx] for i in getattr(self, 'selected_{}_points'.format(met))]) / len(
                        getattr(self, 'selected_{}_points'.format(met)))
                    wandb.log({'active_selection/{}_points_{}'.format(met, cls_idx): stats_point},
    ...
    def save_active_labels(self, selected_frames=None, grad_embeddings=None, cur_epoch=None):
        ...
        #     for met in self.point_measures:
        #         setattr(self, 'selected_{}_points'.format(met),
        #                 [getattr(self, '{}_point_records'.format(met))[i] for i in selected_frames])

        #     with open(os.path.join(self.active_label_dir,
        #                            'selected_frames_epoch_{}_rank_{}.pkl'.format(cur_epoch, self.rank)), 'wb') as f:
        #         pickle.dump({'frame_id': selected_frames, 'selected_mean_points': self.selected_mean_points,
        #                      'selected_bbox': self.selected_bbox, \
        #                      'selected_median_points': self.selected_median_points,
        #                      'selected_variance_points': self.selected_variance_points}, f)

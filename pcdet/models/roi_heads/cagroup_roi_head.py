...
class SimplePoolingLayer(nn.Module):
    def __init__(self, channels=[64,128,128], grid_kernel_size = 5, grid_num = 7, voxel_size=0.04, coord_key=2,
        ...
        # build conv
        self.voxel_size = voxel_size # the voxel_size for stride=2
    ...
    def forward(self, sp_tensor, grid_points, grid_corners=None, box_centers=None, batch_size=None):
        ...
        """
        ## voxelize and conv and recover
        grid_coords = grid_points.long()
        grid_coords[:, 1:4] = torch.floor(grid_points[:, 1:4] / self.voxel_size) # get coords (grid conv center)
        grid_coords[:, 1:4] = torch.clamp(grid_coords[:, 1:4], min=-self.grid_size[0] / 2 + 1, max=self.grid_size[0] / 2 - 1) # -383 ~ 383
        grid_coords_positive = grid_coords[:, 1:4] + self.grid_size[0] // 2  # 0 ~ 766
        ...
        unq_grid_coords[:, 1:4] -= self.grid_size[0] // 2 # -383 ~ 383
        unq_grid_coords[:, 1:4] *= self.coord_key # voxel_size=0.04, for backbone3D voxel_size=0.02
        ...
        new_features = unq_features[unq_inv] ## (b*num_rois*7^3, 128)

        if self.pooling:
            # fake grid
            fake_grid_coords = torch.ones(self.grid_num, self.grid_num, self.grid_num, device=unq_grid_coords.device)
            fake_grid_coords = torch.nonzero(fake_grid_coords) - self.grid_num // 2 # [7**3, 3]
            fake_grid_coords = fake_grid_coords.unsqueeze(0).repeat(grid_coords.shape[0] // fake_grid_coords.shape[0], 1, 1) # (batch*nums_rois, 7**3, 3)
            # fake center
            fake_centers = fake_grid_coords.new_zeros(fake_grid_coords.shape[0], 3) # (batch*nums_rois, 3)
            ...
            fake_center_coords = torch.cat([fake_center_idx, fake_centers], dim=-1).int() # (batch*nums_rois, 4)
            
            fake_grid_idx = fake_batch_idx.reshape([-1, 1, 1]).repeat(1, fake_grid_coords.shape[1], 1) # (batch*nums_rois, 7**3, 1)
            fake_grid_coords = torch.cat([fake_grid_idx, fake_grid_coords], dim=-1).reshape([-1, 4]).int() # (batch*nums_rois*7**3, 4)
...
class CAGroup3DRoIHead(nn.Module):
    def __init__(self, model_cfg, cls_loss_type='BinaryCrossEntropy', reg_loss_type='smooth-l1', **kwargs):
        ...
        mlps = model_cfg.MLPS # [[64,128,128]]
        ...
        for k in range(0, self.reg_fc.__len__()):
            ...
            if k != self.reg_fc.__len__() - 1 and dp_ratio > 0: # 0.3
    ...
    def get_global_grid_points_of_roi(self, rois, grid_size):
        rois = rois.view(-1, rois.shape[-1])  # flatten (B*num, 7)
        batch_size_rcnn = rois.shape[0]
        local_roi_grid_points = self.get_dense_grid_points(rois, batch_size_rcnn, grid_size)  #grid_size=7
        if self.code_size > 6:
        ...
        ## (batch_size*num_max_rois, grid_size**3, 3)
    ...
    def get_dense_grid_points(rois, batch_size_rcnn, grid_size):
        ...
        # like [[-1/2, -1/2, -1/2], ...., [0, 0, 0], ..., [1/2, 1/2, 1/2]]
    ...
    def roi_grid_pool(self, input_dict):
        ...
        middle_feature_list = [input_dict['middle_feature_list'][i] for i in self.middle_feature_source] ## sptensor from backbone3D
        if not isinstance(middle_feature_list, list):
            middle_feature_list = [middle_feature_list]

        ## (batch_size*nums_max_rois, grid_size**3, 3)
    ...
    def forward_train(self, input_dict):
        pred_boxes_3d = input_dict['pred_bbox_list'] # [list1,…, listB], list_i: (bboxes [K_i, 7], scores [K_i], labels [K_i])
        ...
        if self.enlarge_ratio:
            rois[..., 3:6] *= self.enlarge_ratio # False
        ...
        ## assign targets
        ## target: 'rois', 'gt_of_rois', 'roi_labels', 'gt_label_of_rois', 'gt_iou_of_rois', 'roi_scores'
        #### 'reg_valid_mask', 'rcnn_cls_labels'
        targets_dict = self.assign_targets(input_dict)
        input_dict.update(targets_dict)

        # roi pooling
        pooled_features = self.roi_grid_pool(input_dict)  # (batch_size*nums_rois, 128)
        ...
        # self.reg_fc_layers(shared_features): (batch_size*nums_rois, 256)
        rcnn_reg = self.reg_pred_layer(self.reg_fc_layers(shared_features)) # (batch_size*nums_rois, 6/8)
    ...
    def reoder_rois_for_refining(self, pred_boxes_3d):
        ...
        if len(pred_boxes_3d[0]) == 4:
            ...
            use_sem_score = False ## follow this
    ...
    def simple_test(self, input_dict):
        ## normal mode
        if input_dict.get('predict_roi_mode', True):
            ...
            input_dict['rcnn_reg'] = rcnn_reg
            input_dict['rcnn_embedding'] = pooled_features
            ...
            threshold = 0.01
            results_shrink = self.get_boxes(input_dict, img_meta, threshold)
            pred_dict = dict(batch_box_preds=[], batch_score_preds=[], batch_cls_preds=[], batch_embedding=[], batch_roi=[], batch_semantic_score_preds=[],\
            batch_box_preds_shrink=[], batch_score_preds_shrink=[], batch_cls_preds_shrink=[], batch_embedding_shrink=[], batch_roi_shrink=[], batch_semantic_score_preds_shrink=[])
            for i in range(batch_size):
                ...
                pred_dict['batch_embedding'].append(results[i][3])
                pred_dict['batch_roi'].append(results[i][4]),
                pred_dict['batch_semantic_score_preds'].append(results[i][5]),
                pred_dict['batch_box_preds_shrink'].append(results_shrink[i][0])
                pred_dict['batch_score_preds_shrink'].append(results_shrink[i][1])
                pred_dict['batch_cls_preds_shrink'].append(results_shrink[i][2])
                pred_dict['batch_embedding_shrink'].append(results_shrink[i][3])
                pred_dict['batch_roi_shrink'].append(results_shrink[i][4])
                pred_dict['batch_semantic_score_preds_shrink'].append(results_shrink[i][5])
        ...
        ## roi inference mode
            ...
            input_dict['rcnn_reg'] = rcnn_reg
            img_meta = [None for _ in range(batch_size)]
            results = self.get_boxes_simple(input_dict, img_meta)
            pred_dict = dict(batch_roi=[], batch_box_preds=[])
            for i in range(batch_size):
                pred_dict['batch_box_preds'].append(results[i][0])
                pred_dict['batch_roi'].append(results[i][1])
            input_dict.update(pred_dict)

        return input_dict
    
    def get_boxes(self, input_dict, img_meta, thres=None):
        batch_size = input_dict['batch_size']
        rcnn_cls = None
        roi_roi = input_dict['rois']      
        rcnn_reg = input_dict['rcnn_reg']
        roi_labels = input_dict['roi_labels']
        roi_scores = input_dict['roi_scores']
        roi_embedding = input_dict['rcnn_embedding']
        
        roi_sem_scores = input_dict.get('roi_sem_scores', None)


        batch_cls_preds, batch_box_preds = self.generate_predicted_boxes(
                batch_size=batch_size, rois=roi_roi, cls_preds=rcnn_cls, box_preds=rcnn_reg, roi_labels=roi_labels, roi_sem_scores=roi_sem_scores
            ) # [B, num_rois, 7]
        batch_roi_embedding = roi_embedding.reshape(batch_size, -1, roi_embedding.shape[-1])
        ...
        for bs_id in range(batch_size):
            ...
            embeddings = batch_roi_embedding[bs_id]
            semantic_scores = roi_sem_scores[bs_id]
            if not thres:
                result = self._nms(boxes, scores, labels, embeddings, rois, semantic_scores, img_meta[bs_id])
            else:
                result = self._nms(boxes, scores, labels, embeddings, rois, semantic_scores, img_meta[bs_id], thres)
            results.append(result)

        # List(List([N, 7], [N,1], [N, 1]))
        return results

    def get_boxes_simple(self, input_dict, img_meta, thres=None):
        batch_size = input_dict['batch_size']
        roi_roi = input_dict['rois']      
        rcnn_reg = input_dict['rcnn_reg']
        ...
        batch_cls_preds, batch_box_preds = self.generate_predicted_boxes(
                batch_size=batch_size, rois=roi_roi, cls_preds=None, box_preds=rcnn_reg, roi_labels=None, roi_sem_scores=None
            ) # [B, num_rois, 7]
        ...
        for bs_id in range(batch_size):
            ...
            rois = roi_roi[bs_id]
            boxes = batch_box_preds[bs_id]
            yaw_flag = boxes.shape[1] == 7
            if yaw_flag:
                boxes[...,6] *= -1
            else:
                fake_heading = boxes.new_zeros(boxes.shape[0], 1)
                boxes = torch.cat([boxes[:, :6], fake_heading], dim=1)


            rois_mask = (rois[:,0] != 0) & (rois[:,1] != 0)
            results.append([boxes[rois_mask, :], rois[rois_mask, :]])
        # List(List([N, 7], [N,7]))
        return results
    
    def _nms(self, bboxes, scores, labels, embeddings, rois, semantic_scores, img_meta, thres=None):
        if not thres:
            threshold = self.test_iou_thr
        else:
            threshold = thres
        
        n_classes = self.num_class
        yaw_flag = bboxes.shape[1] == 7
        nms_bboxes, nms_scores, nms_labels, nms_embeddings, nms_rois, nms_semantic_scores = [], [], [], [], [], []
        for i in range(n_classes):
            ...
            class_embeddings = embeddings[ids]
            class_rois = rois[ids]
            class_semantic_scores = semantic_scores[ids]
            if yaw_flag:
            ...
            nms_ids, _ = nms_function(class_bboxes, class_scores, threshold)
            nms_bboxes.append(class_bboxes[nms_ids])
            nms_scores.append(class_scores[nms_ids])
            nms_embeddings.append(class_embeddings[nms_ids])
            nms_rois.append(class_rois[nms_ids])
            nms_semantic_scores.append(class_semantic_scores[nms_ids])
        ...
        if len(nms_bboxes):
            ...
            nms_embeddings = torch.cat(nms_embeddings, dim=0)
            nms_rois = torch.cat(nms_rois, dim=0)
            nms_semantic_scores = torch.cat(nms_semantic_scores, dim=0)
        else:
            nms_bboxes = bboxes.new_zeros((0, bboxes.shape[1]))
            nms_scores = bboxes.new_zeros((0,))
            nms_labels = bboxes.new_zeros((0,))
            nms_embeddings = bboxes.new_zeros((0, embeddings.shape[-1]))
            nms_ros = bboxes.new_zeros((0, rois.shape[-1]))
            nms_semantic_scores = bboxes.new_zeros((0, semantic_scores.shape[-1]))
        ...
        return nms_bboxes, nms_scores, nms_labels, nms_embeddings, nms_rois, nms_semantic_scores
    ...
    def generate_predicted_boxes(self, batch_size, rois, cls_preds, box_preds, roi_labels=None, gt_bboxes_3d=None, gt_labels_3d=None, roi_sem_scores=None):
        ...
        local_rois[:, :, 0:3] = 0 ## set local rois coordinates to 0
        batch_box_preds = self.box_coder.decode_torch(batch_box_preds, local_rois).view(-1, code_size) # [B, num_rois, 6/7]
        ...
        batch_box_preds = batch_box_preds.view(batch_size, -1, code_size) # [B, num_rois, 6/7]

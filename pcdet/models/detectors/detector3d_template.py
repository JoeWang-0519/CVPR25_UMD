...
class Detector3DTemplate(nn.Module):
    ...
    def post_processing(self, batch_dict):
        ...
        rcnn_cls = torch.mean(torch.sigmoid(batch_dict['rcnn_cls']), 0).view(batch_size, -1,
                                                                             1) if 'rcnn_cls' in batch_dict.keys() and len(
            batch_dict['rcnn_cls'].shape) > 2 else None

        rcnn_reg = torch.mean(batch_dict['rcnn_reg'], 0).view(batch_size, -1,
                                                              7) if 'rcnn_reg' in batch_dict.keys() and len(
            batch_dict['rcnn_cls'].shape) > 2 else None
        ...
        for index in range(batch_size):
            if batch_dict.get('batch_index', None) is not None:
                ...
                assert batch_dict['batch_box_preds'].shape.__len__() == 3 or batch_dict[
                    'batch_box_preds'].shape.__len__() == 4
                batch_mask = index

            box_preds = batch_dict['batch_box_preds'][:, batch_mask, :, :] if batch_dict[
                                                                                  'batch_box_preds'].shape.__len__() == 4 else \
            batch_dict['batch_box_preds'][batch_mask]
            ...
            for cls_idx in range(len(bbox_classes)):
                ...
                if num_cls_bbox > 0:
                    ...
                    box_idxs_of_pts = roiaware_pool3d_utils.points_in_boxes_gpu(sampled_points,
                                                                                gt_cls_box).long().squeeze(dim=0)
                    unique_pts_counts = torch.tensor(
                        [(box_idxs_of_pts == i).sum() for i in torch.unique(box_idxs_of_pts)])[
                                        1:].float()  # skip the -1

                    mean_points[bbox_classes[cls_idx]] = 0 if torch.isnan(
                        torch.mean(unique_pts_counts)) else torch.mean(unique_pts_counts)
                    median_points[bbox_classes[cls_idx]] = 0 if torch.isnan(
                        torch.median(unique_pts_counts)) else torch.median(unique_pts_counts)
                    variance_points[bbox_classes[cls_idx]] = 0 if torch.isnan(
                        torch.var(unique_pts_counts, unbiased=False)) else torch.var(unique_pts_counts, unbiased=False)

...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
                    rank, tbar, total_it_each_epoch, dataloader_iter, tb_log=None, leave_pbar=False, cur_epoch=None, logger=None):
    ...
    log_buffer = LogBuffer() # log losses
    interval = 50
    ...
    for cur_it in range(total_it_each_epoch):
        ...
        # log losses
        log_buffer.update(disp_dict)
        if (cur_it + 1) % interval == 0:
            log_buffer.average(interval)
        if log_buffer.ready:
            # log
            info = 'Epoch [{0:2d}][{1:4d}]/[{2:4d}] : lr: {3:10.3e}, sem_thr: {4:.2f}, '.format(cur_epoch+1, cur_it + 1, total_it_each_epoch, cur_lr, cur_semantic_value)
...
def resume_dataset(labelled_loader, unlabelled_loader, selected_frames_list, dist_train, logger, cfg):
